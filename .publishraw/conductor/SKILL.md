...
2. Identify the requested operation kind. Reject unknown kinds with OPERATION_KIND_UNKNOWN. Local draft discard is lifecycle handling, never item/media deletion. Call discardLocalDraft from the verified adapterRunnerEntry only for the identified operation. A clean discard removes only its draft, operation and recovery artifacts and always preserves local source images. Published content, an open PR or a public asset leaves a durable local receipt; discard never undoes them. Publishing or OUTCOME_UNVERIFIED blocks discard until the remote outcome is settled, so the evidence needed for reconciliation survives.
